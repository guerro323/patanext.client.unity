...
public class OutlinePass : ScriptableRenderPass
{
    ...
    public OutlinePass(OutlineFeature.OutlineSettings settings)
    {
        ...
        filteringSettings = new FilteringSettings(RenderQueueRange.all, -1, 1u << settings.layer - 1);
         ...
         if(!outlineMaterial) outlineMaterial = CoreUtils.CreateEngineMaterial(shaderName); 
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        using (new ProfilingScope(cmd, profilingSampler))
        {
            ...
            cmd.Blit(nearestPointID, renderingData.cameraData.renderer.cameraColorTarget, outlineMaterial, 4);
        }
...
if(!stencilMaterial) stencilMaterial = CoreUtils.CreateEngineMaterial(stencilShaderName); 
